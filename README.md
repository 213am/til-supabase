...
```json
  "scripts": {
    ...
    "start": "next start",
    "lint": "next lint",
    "generate-types": "npx supabase gen types typescript --project-id 프로젝트아이디 --schema public > src/types/types_db.ts" // 추가
  },
```
...
- `/src/types/types_db.ts` 파일이 자동으로 생성됨
...
- `/src/middleware.ts` : Next 에서 역할이 정해진 파일명
- 임시로 `_middleware.ts` 로 파일명 수정
